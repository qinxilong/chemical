    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['chemical:txtAvg:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['chemical:txtAvg:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['chemical:txtAvg:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['chemical:txtAvg:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listTxtAvg, getTxtAvg, delTxtAvg, addTxtAvg, updateTxtAvg } from "@/api/chemical/txtAvg";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listTxtAvg(this.queryParams).then(response => {
        this.txtAvgList = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加TXT发送数据";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const id = row.id || this.ids
      getTxtAvg(id).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改TXT发送数据";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateTxtAvg(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addTxtAvg(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除TXT发送数据编号为"' + ids + '"的数据项？').then(function() {
        return delTxtAvg(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('chemical/txtAvg/export', {
        ...this.queryParams
      }, `txtAvg_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
